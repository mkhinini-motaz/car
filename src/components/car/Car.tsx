...
export default function Car({ data, showDetails = false }: CarProps): JSX.Element {
  return (<Section>
    ...
    <LangAwareView className="justify-between items-center">
      <LangAwareView>
        <MaterialIconsIcon name="numbers" size={30} />
        <Text className={'text-lg'}>{data.licence_plate}</Text>
      </LangAwareView>

      {showDetails ? <LangAwareView className="gap-x-2 items-center justify-between">
